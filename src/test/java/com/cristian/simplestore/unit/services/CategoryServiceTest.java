...
public class CategoryServiceTest extends BaseTest {
	...
	public void testItfindsAllCategories() {
		...
		List<CategoryEntity> categories = categoryGenerator.saveRandomCategoriesOnDb(CATEGORIES_SIZE);

		List<CategoryEntity> foundCategories = categoryService.findAll();
	...
	public void testItfindACategoryById() {
		...
		CategoryEntity foundCategory = categoryService.findById(category.getId());
	...
	public void testItcreatesACategoryWithForm() {
		...
		CategoryEntity createdCategory = categoryService.create(categoryCreateData);
	...
	public void testItupdatesACategoryWithForm() {
		...
		CategoryEntity updatedCategory = categoryService.update(categoryUpdateData);
	...
	public void testItCorrectlyUpdatesTheParentCategory() {
		// B -> A
		CategoryEntity categoryA = categoryGenerator.new Builder().randomName().randomParent().save();
		CategoryEntity categoryB = categoryA.getParentCategory();
		...
		CategoryEntity expectedCategoryB = categoryService.update(updateCategoryBdata);
		CategoryEntity expectedCategoryA = categoryService.findById(categoryA.getId());
	...
	public void testItDeletesACategoryImage() {
		...
		CategoryEntity updatedCategory = categoryService.update(newCategoryData);
	...
	public void delete() {
		CategoryEntity categoryToDelete = categoryGenerator.saveRandomCategoryOnDb();
	...
	private void assertThatTwoCategoriesAreEqual(CategoryEntity c1, CategoryEntity c2) {
