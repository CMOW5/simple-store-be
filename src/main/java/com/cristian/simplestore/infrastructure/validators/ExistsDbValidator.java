...
public class ExistsDbValidator implements ConstraintValidator<ExistsDb, Object> {
  ...
  public boolean isValid(Object value, ConstraintValidatorContext context) {
    ...
    if (exceptIdColumn.isEmpty()) {
      ...
      return !entryExistsExceptWithId(table, columnName, uniqueFieldValue, exceptIdColumn,
          exceptIdValue);
    }
  }
  ...
  /**
   * verify if a record has the given column value for the given column name in a the given table,
   * if the given record has the given id in the id column name, the verification is excluded
   * 
   * @param table
   * @param columnName
   * @param columnValue
   ...
   * @return true if the entry exists
   */
  private boolean entryExistsExceptWithId(String table, String columnName, String columnValue,
      String idColumnName, Object idColumnValue) {
    String query = "SELECT " + columnName + " FROM " + table + " WHERE " + columnName + "= ?"
        + " AND " + idColumnName + " != ?";
    Query q = entityManager.createNativeQuery(query);
    q.setParameter(1, columnValue);
